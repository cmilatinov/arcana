...
public class HostMenu {
	...
	private String[] items = { "Map", "Gamemode", "Back" };
	...
	public HostMenu(JFrame frame, Settings s) {
		this.frame = frame;
		...
		int startY = (frame.getContentPane().getHeight() / 2) + (((frame.getContentPane().getHeight() / 2)
				- (((((int) fm.getStringBounds("A", frame.getGraphics()).getHeight()) + spaceBetweenItems)
						* items.length)))
				/ 2);
